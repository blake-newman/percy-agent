...
class AssetDiscoveryService extends percy_client_service_1.default {
    ...
    async createPagePool(browser, size) {
        logger_1.profile('-> assetDiscoveryService.newPagePool');
    ...
    async discoverResources(rootResourceUrl, domSnapshot, options) {
        ...
        if (!this.isParallel && this.pages === null) {
            logger_1.default.error('Failed to create pool of pages in sequential run.');
            return [];
        }
        ...
        for (let idx = 0; idx < widths.length; idx++) {
            ...
            const promise = this.resourcesForWidth(this.browser, page, width, domSnapshot, rootResourceUrl, enableJavaScript);
    ...
    async resourcesForWidth(browser, pooledPage, width, domSnapshot, rootResourceUrl, enableJavaScript) {
        ...
        try {
            ...
            logger_1.profile('--> assetDiscoveryServer.waitForResourceProcessing');
            if (this.isParallel) {
                logger_1.profile('--> assetDiscoveryService.page.close', { url: rootResourceUrl });
                await page.close();
                logger_1.profile('--> assetDiscoveryService.page.close');
            }
